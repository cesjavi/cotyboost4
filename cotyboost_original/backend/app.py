...
def load_inference_model(project_id):
    if project_id not in inference_sessions:
        adapter_path = f"temp_projects/{project_id}/adapter"
        analysis_path = f"temp_projects/{project_id}/analysis.json"
        if os.path.exists(analysis_path):
            with open(analysis_path) as f:
                analysis = json.load(f)
                model_name = analysis.get("suggested_model", "Salesforce/codegen-6B-mono")
        else:
            model_name = "Salesforce/codegen-6B-mono"

        print(f"🔍 Cargando modelo desde: {adapter_path} (base: {model_name})")

        # Crear un directorio temporal de offload (si no existe)
        offload_dir = f"temp_projects/{project_id}/offload"
        ...
        # ¡Aquí agregamos low_cpu_mem_usage y offload_folder!
...
def inference():
    ...
    # Nuevo: permite especificar longitud máxima generada
    ...
    try:
        ...
        # Tokenización con attention_mask, padding, truncation, longitud máxima del modelo base (generalmente 2048)
        inputs = tokenizer(
            prompt,
            return_tensors="pt",
            padding=True,
