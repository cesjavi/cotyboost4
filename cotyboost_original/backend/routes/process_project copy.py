...
import os, uuid, zipfile, shutil, glob
...
def process_project():
    try:
        ...
        project_name = None
        # Extract project name and normalize it
        ...
        # GitHub URL
         ...
         #3. Local folder path
            ...
            # Copiar todo el contenido a extract_path
        ...
        # Construcción del dataset básico
        dataset = []
        for ext in ['*.py', '*.cs']:
...
def download_adapter(project_id):
    folder_path = os.path.join("temp_projects", project_id, "adapter")
    adapter_path = os.path.join(folder_path, "adapter_model.safetensors")
    if os.path.exists(adapter_path):
        return send_file(adapter_path, as_attachment=True)
    else:
